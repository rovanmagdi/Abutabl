        ...
        <>
            ...
            <ModuleView header={<Header />} >
                {todoListData?.todoListData?.allAssigns?.map((item: { data: [], date: string }) => {
                    return (
                        <>
                            {item?.data?.length !== 0 && <Flex className="align-center flex-wrap ">
                                <Text className='mb-3 font-bold'>{item?.date}</Text>
                                {item?.data?.map((item: { by: string, status: string, subject_name: string, type: string, date: string, path: string }) => {
                                        ...
                                        <Box className="border border-Platinum rounded-xl w-[100%] p-5 flex  justify-between">
                                            <Flex>
                                                <Box className="bg-Cultured p-10 rounded-xl">
                                                    <Text className="bg-New absolute top-3 right-3 text-sm px-3 text-TextNew">{item?.status}</Text>
                                                    <Assign />
                                                </Box>
                                                <Box className="mx-5 gap-4 flex flex-col ">
                                                    ...
                                                    <Box >
                                                        ...
                                                        <Text className='flex pt-3'> <img src={Man} width={30} className='pr-1' />{item?.by}</Text>
                                                    </Box>
                                                </Box>
                                            </Flex>
                                            <Box className="mt-3">
